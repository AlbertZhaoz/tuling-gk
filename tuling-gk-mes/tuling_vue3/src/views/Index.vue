  ...
  <div id="vol-container" :class="['vol-theme-' + theme]">
    <div class="vol-aside" :style="{ width: menuWidth + 'px' }">
      ...
      <div class="vol-menu">
        <el-scrollbar style="height: 100%">
          <VolMenu :currentMenuId="currentMenuId" :on-select="onSelect" :enable="true" :open-select="false" :isCollapse="isCollapse" :list="menuOptions"></VolMenu>
        </el-scrollbar>
      </div>
    </div>
    <div class="vol-container" :style="{ left: menuWidth - 1 + 'px' }">
      <div class="vol-header">
        <div class="project-name">工厂智能化管理系统</div>
        <div class="pdf" @click="openPdf">帮助手册</div>
      ...
      <div class="vol-path">
        <el-tabs @tab-click="selectNav" @tab-remove="removeNav" @contextmenu.prevent="bindRightClickMenu(false)" type="border-card" class="header-navigation" v-model="selectId" :strtch="false">
          <el-tab-pane v-for="(item, navIndex) in navigation" type="card" :name="navIndex + ''" :closable="navIndex > 0" :key="navIndex" :label="item.name">
    ...
    <el-drawer title="选择主题" v-model="drawer_model" direction="rtl" destroy-on-close>
      <div class="theme-selector">
        <div @click="changeTheme(item.name)" class="item" v-for="(item, index) in theme_color" :key="index" :style="{ background: item.color }">
          <div v-show="item.leftColor" :style="{ background: item.leftColor }" style="height: 100%; width: 20px" class="t-left"></div>
    ...
    <el-drawer title="消息列表" v-model="messageModel" direction="rtl" destroy-on-close>
      <Message :list="messageList" @clearAllMessage="clearAllMessage"></Message>
    </el-drawer>
  </div>
</template>
...
export default defineComponent({
  ...
  setup(props, context) {
    ...
    const links = ref([
      ...
      { text: "设备监控", path: "/DeviceMonitor", id: -3 },
    ...
    const to = (item) => {
      ...
      // 设置弹出的时候自动打开新页面
      if (item.path == "/kanban") return window.open("/#/kanban", "_blank");
      if (item.path == "/DeviceMonitor")
        return window.open("/#/DeviceMonitor", "_blank");
