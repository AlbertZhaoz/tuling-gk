...
let methods = {
  ...
  async add() {
    ...
    //新建
    if(this.table.cnName == "设备保养列表") {
      if(!this.$store.getters.data().MaintainancePkInt) {
        return this.$message.error("请选择设备保养行，再添加设备")
      }
    }
    if(this.table.cnName == "设备维修列表") {
      if(!this.$store.getters.data().RepairPkInt) {
        return this.$message.error("请选择设备维修行，再添加设备")
      }
    }
    console.log(this.table.cnName);
    this.currentAction = this.const.ADD;
    this.currentRow = {};
    if (!(await this.initBox())) return;
  ...
  export(isDetail) {
    ...
    if (isDetail) {
      ...
      console.log(this.$refs.table.getSelected(),1111)
      ...
      selectRows.forEach(item => {
        query.wheres.push({
          name: this.table.key,
          value: this.table.key == undefined || this.table.key == null ? '' : item[this.table.key],
          // displayType: 'like'
        });    
      });
