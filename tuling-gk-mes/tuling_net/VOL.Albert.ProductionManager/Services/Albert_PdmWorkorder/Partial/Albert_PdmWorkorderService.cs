/*
 *所有关于Albert_PdmWorkorder类的业务代码应在此处编写
*可使用repository.调用常用方法，获取EF/Dapper等信息
*如果需要事务请使用repository.DbContextBeginTransaction
*也可使用DBServerProvider.手动获取数据库相关信息
*用户信息、权限、角色等使用UserContext.Current操作
*Albert_PdmWorkorderService对增、删、改查、导入、导出、审核业务代码扩展参照ServiceFunFilter
*/
...
{
    public partial class Albert_PdmWorkorderService
    {
        private readonly IHttpContextAccessor _httpContextAccessor;
        private readonly IAlbert_PdmWorkorderRepository _repository;//访问数据库
        private readonly IAlbert_BasicNumberRuleRepository _basicNumberRuleRepository;// 编码规则数据库

        [ActivatorUtilitiesConstructor]
        public Albert_PdmWorkorderService(
            IAlbert_PdmWorkorderRepository dbRepository,
        ...
        public override WebResponseContent Add(SaveModel saveDataModel)
        {
            ...
            AddOnExecuting = (Albert_PdmWorkorder workorder, object list) =>
            {
                if (string.IsNullOrWhiteSpace(workorder.WorkorderCode) || workorder.WorkorderCode.Contains("自动生成"))
                    workorder.WorkorderCode = GetProductCode();
                //如果返回false,后面代码不会再执行
