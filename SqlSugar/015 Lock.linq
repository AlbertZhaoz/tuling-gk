...
void Main()
{
	try
	{
		...
		try
		{
			...
			// 查询更新
			var df=db.Queryable<albert_datafirst>().TranLock(DbLockType.Wait)
			.Where(it=>it.DataPkInt==3).First();
			df.ProductCode = "666";
			db.Updateable(df).ExecuteCommand();
			db.Ado.CommitTran();
			
			
			// df version
			// datapkint 3 ProductCode=777   version=1
			// datapkint 3 ProductCode=666   version=2
			
			//  ProductCode=777   
			// datapkint 3 ProductCode=666   version=2
			// select xxx from xxx where datapkint=3 and version=1
			
			
			// update item set q=q-1 where id =1 and q-1>0
			// set num+1
