...
void Main()
{
	try
	{
		...
		//var list = db.Queryable<albert_datafirst>()
		//.Where(a => a.DataPkInt<5)
		//.ToList()
		//;
		//
		//// System.Text.Json
		...
		//5.0.5.1 Json直接序列化
//		var json = File.ReadAllText(@"F:\01_Repo\tuling-gk\SqlSugar\013.json");
//		
//		var conModels = db.Utilities
//		.JsonToConditionalModels(json);
//        db.Queryable<albert_datafirst>()
//		.Where(conModels).ToList().Dump();

var list = db.Queryable<dynamic>().AS("albert_datafirst")
.Where("DataPkInt=@DataPkInt", new { DataPkInt = 4 }).ToList();
list.Dump();
	}
	catch (Exception ex)
	{
