...
void Main()
{
	try
	{
		...
		//db.Queryable<albert_datafirst>().Where(a => a.ProductCode=="4201042316107607")
		//.ToList()
		//.Dump();

		Expression<Func<albert_datafirst, bool>> exp = 
		Expressionable.Create<albert_datafirst>() //创建表达式
			  .AndIF(true,df=>df.ProductCode == "4201042316107607")
			  .OrIF(true,df=>df.ProductCode == "4201042316117759")
			  .ToExpression();//注意 这一句 不能少

		var list = db.Queryable<albert_datafirst>().Where(x=>x.ProductCode=="xxx").ToList()
		.Dump();//直接用就行了不需要判段 null和加true
