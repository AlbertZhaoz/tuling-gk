...
// 自定义的组件
import gridFooter from './Albert_DeviceMaintainance_Footer.vue';
...
let extension = {
    ...
    methods: {
        ...
        rowClick({ row, column, event }) {
            ...
            //缓存当前选中行的主键id
            this.$store.getters.data().MaintainancePkInt = row.MaintainancePkInt;
            ...
            this.editFormOptions.forEach(options => {
                options.forEach(item => {
                    if (this.currentAction == "Add") {
                        if (item.field == "MaintainanceStartDate") {
                            // 格式化时间字符串使用方法见 js 封装函数 utils/dateFormUtil.js
                            //this.editFormFields.MaintainanceStartDate = dateUtil.getNowDate("yyyy-MM-dd hh:mm:ss");
                            this.editFormFields.MaintainanceStartDate = dateUtil.getNowDate();
                        }
                        // 判断字段是否包含某字符串 
