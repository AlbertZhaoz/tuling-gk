...
namespace VOL.System.Services
{
    public partial class Sys_UserService
    {
        ...
        public Sys_UserService(IHttpContextAccessor httpContextAccessor, ISys_UserRepository repository, ISys_RoleRepository roleRepository)
            : base(repository)
        {
        ...
        /// <returns></returns>
        public async Task<WebResponseContent> Login(LoginInfo loginInfo, bool verificationCode = false)
        {
            ...
            try
            {
                ...
                if (user == null)
                {
                    ...
                    var role = await _roleRepository.FindAsIQueryable(x => x.RoleName == "访客")
                        .FirstOrDefaultAsync();

                    user = new Sys_User()
                    {
                        UserName = loginInfo.UserName,
                        UserTrueName = loginInfo.UserName,
                        UserPwd = pwd.EncryptDES(AppSetting.Secret.User),
                        Role_Id = (int)(role?.Role_Id),
                        RoleName = "访客",
                        IsRegregisterPhone = 1,
                        Enable = 1,
                        CreateDate = DateTime.Now,
                    };
                    repository.DbContext.Set<Sys_User>().Add(user);
                ...
                {
                    if (loginInfo.Password.Trim().EncryptDES(AppSetting.Secret.User) != (user.UserPwd ?? ""))
